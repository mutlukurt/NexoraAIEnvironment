...
// ---- 1) frontierBuildSystemPrompt ----
{
  ...
  writeFileSync(entry, `export { frontierBuildSystemPrompt } from '${join(repo, 'electron/shared/prompts.ts')}'\n`)
  await build({ entryPoints: [entry], bundle: true, format: 'esm', platform: 'node', outfile })
  const { frontierBuildSystemPrompt } = await import(pathToFileURL(outfile).href)
