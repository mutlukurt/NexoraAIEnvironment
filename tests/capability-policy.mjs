...
check('auto tier requires approval for package execution', !authorizeCommand('npm install', { tier: 'auto' }).allowed)
check('explicit approval permits ask-class command', authorizeCommand('npm install', { tier: 'auto', approved: true }).allowed)
check('project-wide grant permits ask-class command', authorizeCommand('npm run build', { tier: 'auto', projectAlways: true }).allowed)
