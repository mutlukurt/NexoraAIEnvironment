...
8. **Real-Time Hardening** (v1.4) — 🚧 **planned: the clock is the enemy.** Phase 7's very first end-to-end run on the real app with a real model (the Zeytin Kafe live test) exposed the class the browser harness cannot see by design: **real-duration races and a busy inference server** — the same code that froze in production ran itself under mock timing. Seven items, each pinned to a live-test failure: a **stream-liveness sentinel + absolute Durdur + real llama-server cancellation** (a penalty turn once hung at 0 bytes for 36 minutes and restart was the only exit), a **queue heartbeat** (delegated tasks sat frozen at "sırada" because the single-shot trigger raced minutes-long verify chains), a **behavior walk that retries** (its one-shot 12s timer silently died against a running repair turn), **goal-met verdicts** ("verified" must mean the request happened, not just that it compiled), **project identity** (planned builds get a real package.json so knowledge/rules/history stop piling under the default folder), the small honest fixes from the ledger, and a **test surface that can see time** (timing-realistic browser mode + a real-engine `test:e2e` covering queue-under-load, abort-against-busy-server, timer-vs-repair races). Full plan in [ROADMAP.md](ROADMAP.md).
