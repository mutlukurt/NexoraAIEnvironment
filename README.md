...
**Example output (latest)** — **VOLTA**, an electric-mobility brand built by a **local 3B** from a single prompt, then finished with two chat iterations — the Phase 9 fidelity pipeline (contract-driven plan, verbatim slotting, Tailwind v4 scaffold), shown from a real `vite build`:

![VOLTA — one prompt + two iterations, local 3B](docs/screenshots/volta-fidelity-live.png)

---

**The first runs** — the earliest projects this app was ever tested on, kept for the record (oldest at the bottom). First, a complete restaurant site (17-file professional structure: section components, `ui/` primitives, a typed data layer, 9 priced menu items, 6 starred customer reviews) generated by a **real Qwen2.5-Coder-14B** from a detailed client-style brief, then refined through the app's own surgical-edit iteration loop:
