...
> **🩹 v0.20.1 patch:** fixed a startup bug where a leftover queued or scheduled task could auto-fire the moment the app opened — before any model was ready — producing a red *"no model loaded, API turn failed"* error even though the user did nothing. Now the queue **never runs a turn without a ready engine**, restored queues start **paused** (resume with your next message), and a failed auto-turn reverts cleanly instead of erroring.
