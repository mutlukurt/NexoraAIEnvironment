...
| **Verification** | — | live-verified in a **browser-preview harness** (real React components + real Zustand stores, mocked IPC): model picked → loaded → chat enabled, ⚙ toggles, theme flip (dark↔light), view switch — zero console errors, `typecheck:web` clean |

The next release (v0.14.3) returns to the engine — wiring the update-turn prompt to the whole-file policy so a small model finally gets the escape hatch v0.14.1 built for it.
