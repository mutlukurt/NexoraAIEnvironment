  ...
  if (node.isDir) {
    return (
      <div>
        <button
          onClick={() => setOpen((o) => !o)}
          className="flex w-full items-center gap-2 py-2 pr-3 text-left text-sm font-bold text-slate-500 hover:bg-slate-50 transition"
  ...
  return (
    <button
      ...
      className={
        ...
        (active
          ? 'bg-brand-50/70 text-brand-600 font-bold border-l-2 border-brand-500'
  ...
  return (
    <div className="flex h-full flex-col bg-white">
      <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3 bg-slate-50/50">
        ...
        <div className="flex gap-1.5">
          <button
            ...
            className="border border-slate-200 bg-white rounded-lg px-2.5 py-1 text-xs font-bold text-slate-500 hover:border-slate-350 hover:text-slate-700 shadow-sm transition flex items-center gap-1"
          >
      ...
      <div className="flex-1 overflow-y-auto py-2">
        {tree.length === 0 ? (
          <p className="px-4 py-8 text-center text-xs text-slate-400 font-semibold">{t.noChats === 'Henüz sohbet yok' ? 'Henüz dosya yok' : 'No files yet'}</p>
        ) : (
          tree.map((n) => (
            <Row
              key={n.path}
      ...
      {selectedPath && (
        <button
          ...
          className="border-t border-slate-200 px-4 py-2.5 bg-slate-50/50 text-left text-xs font-bold text-slate-400 hover:text-red-500 hover:bg-red-50/50 transition flex items-center gap-1.5"
        >
